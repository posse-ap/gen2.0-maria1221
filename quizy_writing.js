...
function question(questionNumber, quizSetList, correctNumber) {
  let questionContent =
    ...
    `<h1 class="question">${questionNumber}. この地名はなんて読む？</h1>` +
    `<img class="questionImg" src="image/${questionNumber}.png">` +
  ...
  // 解答ボックス
  // answerTextというidがついたpタグの中身はクリックしたときに「正解」もしくは「不正解」と表示される
   questionContent +=
    `<div id="answerBox_${questionNumber}" class="answerBox">` +
    `<p id="answerText_${questionNumber}"></p>` +
    `<p class="answer">正解は「${quizSetList[correctNumber]}」です！</p>` +
    "</div>"
    + "</div>";
...
// indexOf questionChoicesでcorrectが一番最初に出てきたindex
})
}
...
function select (questionNumber, selectNumber, correctNumber) {
  ...
  if (selectNumber == correctNumber) {
    ...
    answerText.classList.add("incorrectText")
    correctChoice.classList.add("correct");
