...
for (let k = 0; k < quizSetLength; k++) {
  let questionContent =
    ...
    '.png" alt=問いとなる地名の画像 />' +
    `<ul id='choices${k}'>` +
    // `<li class="choice" id="incorrectChoice${k}_1">` +
    //   quizSet[k][1] +
    // "</li>" +
    ...
    //   quizSet[k][0] +
    // "</li>" +
    // `<li class="choice" id="incorrectChoice${k}_2">` +
    // quizSet[k][2] +
    // "</li>" +
    "</ul>" +
    "</div>";
...
function shuffle(array) { // 引数arrayに配列を渡して、それをシャッフルして返す
...
//配列をコピーしてそれを[]で囲い新たに配列化させたものをシャッフルに使うことで、元の配列には影響なくシャッフルした配列を用意
shuffledChoices.forEach(choiceShuffle => {  //変数choiceShuffleにshuffledChoicesを一つずつ入れる
  const li = document.createElement('li');//li要素の作成
  li.textContent = choiceShuffle;//choiceにchoiceShuffleを入れる
  li.classList.add('choice');
  let choices = document.getElementById(`choices${i}`);
  choices.appendChild(li);//choicesというidがついたul要素の下に作成

  // 正解と不正解の選択肢にそれぞれid属性を追加する
  for (let i = 0; i < quizSetLength; i++) {
    if (li.textContent === quizSet[i][0]){
       let correct = "correct"; //付与するid名
...
//       let correctText = document.createElement("p");
//       correctText.innerText = `正解は「 ${quizSet[i]} 」です！`;
//       answerBox.appendChild(correctText);
//       incorrectChoice1.style.pointerEvents = "none";
//       incorrectChoice2.style.pointerEvents = "none";
//       correctChoice.style.pointerEvents = "none";
